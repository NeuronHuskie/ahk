...
class JSON_VIEWER_CLASS {
    ...
    __New(json_input, options?) {
        ...
        this.json_string        := IsObject(json_input) ? JSON.Dump(json_input) : FileExist(json_input) ? FileRead(json_input, 'UTF-8-RAW') : json_input
        this.gui_width          := IsSet(options) && options.HasOwnProp('width') ? options.width : (A_ScreenWidth * 0.7)
        this.gui_height         := IsSet(options) && options.HasOwnProp('height') ? options.height : (A_ScreenHeight * 0.7)
        this.title              := IsSet(options) && options.HasOwnProp('title') ? options.title : 'json viewer [ahk]'
        this.view_mode          := IsSet(options) && options.HasOwnProp('view') ? options.view : 'tree'
        this.submit_on_enter    := IsSet(options) && options.HasOwnProp('submit_on_enter') ? options.submit_on_enter : false
        ...
        try {
            ...
            this.wv.AddHostObjectToScript('select_handler', this.handle_select.Bind(this))
            this.wv.AddHostObjectToScript('copy_handler', this.handle_copy.Bind(this))
            this.wv.AddHostObjectToScript('close_handler', this.handle_close.Bind(this))
            this.wv.AddHostObjectToScript('open_url_handler', this.handle_open_url.Bind(this))
            this.wv.AddHostObjectToScript('save_file_handler', this.handle_save_file.Bind(this))
            this.wv.AddHostObjectToScript('open_path_handler', this.handle_open_path.Bind(this))
            this.wv.AddHostObjectToScript('check_path_handler', this.handle_check_path.Bind(this))
            this.wv.AddHostObjectToScript('read_file_handler', this.handle_read_file.Bind(this))

            if (this.wv.HasMethod('InjectAhkComponent'))
                this.wv.InjectAhkComponent()

            this.g.OnEvent('Close', this.on_close.Bind(this))
            this.g.OnEvent('Size', this.on_resize.Bind(this))

            this.wv.NavigateToString(this._build_html())
    ...
    on_close(*) => this.dialog_closed := true
    ...
    _build_html() {
        ...
        html := StrReplace(html, '{{JSON_DATA}}', this.json_string)
        html := StrReplace(html, '{{VIEW_MODE}}', this.view_mode)
        html := StrReplace(html, '{{TITLE}}', this.title)
        html := StrReplace(html, '{{SUBMIT_ON_ENTER}}', this.submit_on_enter)
        return html
    }
